...
def convert_office_to_pdf(source_path: Path, out_dir: Path) -> Path:
    ...
    # Con esto podemos tener dos conversiones simultaneas abiertas sin chocar por los candados del perfil
    ...
    try:
        ...
        result = subprocess.run(
            ...
            timeout=120,          # aborta si tarda más de 120 s
            check=False,          # no lanzar excepción solo por returncode != 0
        )
    except subprocess.TimeoutExpired:
        raise RuntimeError("La conversión tardó más de 120 segundos.")
